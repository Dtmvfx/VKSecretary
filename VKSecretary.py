...
def incoming_requests_vk(page, signals):
    try:
        signals.log_signal.emit(f'\n~~~~~~~~~~~~~~~~~~~~~~ СМОТРИМ ВХОДЯЩИЕ ЗАЯВКИ В ДРУЗЬЯ [ВК] ~~~~~~~~~~~~~~~~~~~~~~\n            Блок по дефолту: ЧС // СТОП-СЛОВО [{', '.join(stop_words)}] // ВОЗРАСТ (<20) // ЛИМИТ ДРУЗЕЙ (<500)\n')
...
def outgoing_requests_vk(page, signals):
    try:
        signals.log_signal.emit(f'\n~~~~~~~~~~~~~~~~~~~~~~ ПОСЫЛАЕМ ИСХОДЯЩИЕ ЗАЯВКИ В ДРУЗЬЯ [ВК] ~~~~~~~~~~~~~~~~~~~~~~\n          Блок по дефолту: ЧС // СТОП-СЛОВО [{', '.join(stop_words)}] // ВОЗРАСТ (<20) // ЛИМИТ ДРУЗЕЙ (<500) // ЗАКРЫТЫЙ ПРОФИЛЬ\n')
        out_requests = 0
